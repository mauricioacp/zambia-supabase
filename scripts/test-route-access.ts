...
import { SUPABASE_ANON_KEY, SUPABASE_URL } from "../_environment.ts";
...
async function testRouteAccess(route: string, requiredLevel: number) {
	...
	for (const user of testUsers) {
		...
		console.log(
			`${yellow}Testing with ${user.role} (Level ${level})...${reset}`,
		);
		...
		try {
			...
			const response = await fetch(
				`http://127.0.0.1:54321/functions/v1${route}`,
				{
					method: "POST",
					headers: {
			...
			results.push({
				...
				message: shouldHaveAccess
					? accessDenied
						...
						: `Access granted (${response.status})`
					: accessDenied
						? `Correctly denied (${response.status})`
						: `Unexpected access (${response.status})`,
			});

			if (success) {
				console.log(
					`  ${green}✓ ${user.role}: ${results[results.length - 1].message}${reset}`,
				);
			} else {
				console.log(
					`  ${red}✗ ${user.role}: ${results[results.length - 1].message}${reset}`,
				);
			}
		} catch (error) {
...
async function main() {
	...
	console.log(
		`Run: ${green}npx supabase functions serve akademy-app --env-file .env.local${reset}`,
	);
