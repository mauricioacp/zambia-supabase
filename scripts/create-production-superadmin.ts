...
if (!SUPABASE_URL || SUPABASE_URL.includes("your-project")) {
	console.error(
		"❌ Please set SUPABASE_URL environment variable or update the script",
	);
	Deno.exit(1);
}
...
if (
	...
	console.error(
		"❌ Please set SUPABASE_SERVICE_ROLE_KEY environment variable or update the script",
	);
	Deno.exit(1);
}
...
async function createSuperAdmin() {
	try {
		...
		if (hqError)
			throw new Error(`Failed to fetch headquarter: ${hqError.message}`);
		...
		if (roleError)
			throw new Error(`Failed to fetch superadmin role: ${roleError.message}`);
		...
		if (existingUser) {
			...
			console.log(
				"ℹ️  If you need to reset this user, please delete it first from the Supabase dashboard",
			);
			Deno.exit(1);
		}
		...
		console.error(
			"1. Make sure you have the correct SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY",
		);
		console.error("2. Ensure the database has been reset and seeded");
		console.error(
			"3. Check that roles, headquarters, and seasons tables have data",
		);
		Deno.exit(1);
	}
}
