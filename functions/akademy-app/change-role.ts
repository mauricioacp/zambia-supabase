...
import { type ChangeRoleResponse, ChangeRoleSchema } from "./user.ts";
...
export async function changeUserRole(c: Context): Promise<Response> {
	...
	try {
		...
		const { data: originalAgreement, error: originalError } =
			await supabaseAdmin
				.from("agreements")
		...
		if (targetRole.level > userLevel) {
			throw new HTTPException(403, {
				message: `Cannot change to role level ${targetRole.level}. Lack of permissions`,
			});
		}
