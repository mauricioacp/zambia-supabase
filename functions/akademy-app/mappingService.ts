...
import { formatIsoDate, isCreatedMoreThanOneYearAgo } from "./helpers.ts";
import type { StrapiAgreement, SupabaseAgreement } from "./interfaces.ts";
...
export async function matchData(
	...
	rolesSet.forEach((role) => {
		...
		mergedRoles.set(normalizeText(role), rolesMap.get(normalizedRole) || null);
	});
	...
	for (const strapiAgreement of strapiAgreements) {
		...
		if (!hqFromMerged) {
			const normalizedHqForMapping = normalizeHeadquarters(
			...
			if (!mappedHeadquarters) {
				console.error(
					`[ERROR] Agreement ID ${strapiAgreement.id}: Headquarters "${strapiAgreement.headQuarters}" not found in mapping`,
				);
		...
		if (!roleFromMerged) {
			...
			if (!mappedRole) {
				console.error(
					`[ERROR] Agreement ID ${strapiAgreement.id}: Role "${strapiAgreement.role}" not found in mapping`,
				);
	...
	if (supabaseAgreements.length > 0) {
		...
		const documentNumbers = supabaseAgreements.map(
			(agreement) => agreement.document_number,
		);
