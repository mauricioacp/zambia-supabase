...
async function checkExistingAgreementsByEmail(
  ...
  console.log(`Processing ${emails.length} emails in ${emailBatches.length} batches`);
  
  for (let i = 0; i < emailBatches.length; i++) {
    const batch = emailBatches[i];
    console.log(`Processing email batch ${i + 1}/${emailBatches.length} (${batch.length} emails)`);
    ...
    try {
      ...
      if (error) {
        console.error(`Error checking for existing emails in batch ${i + 1}:`, error);
        throw error;
      }
...
async function checkExistingAgreementsByDocumentNumber(
  ...
  console.log(`Processing ${remainingDocumentNumbers.length} document numbers in ${docBatches.length} batches`);
  
  for (let i = 0; i < docBatches.length; i++) {
    const batch = docBatches[i];
    console.log(`Processing document number batch ${i + 1}/${docBatches.length} (${batch.length} document numbers)`);
    ...
    try {
      ...
      if (error) {
        console.error(`Error checking for existing document numbers in batch ${i + 1}:`, error);
        throw error;
      }
      ...
      if (data && data.length > 0) {
        ...
        data.forEach(docResult => {
          if (!existingAgreements.some(existing => 
            existing.email.toLowerCase() === docResult.email.toLowerCase() && 
            existing.document_number.toLowerCase() === docResult.document_number.toLowerCase()
          )) {
...
export async function checkExistingAgreementsInBatches(
  ...
  const existingAgreementsByDocNumber = await checkExistingAgreementsByDocumentNumber(
    supabaseClient,
    documentNumbers,
  ...
  existingAgreementsByDocNumber.forEach(docAgreement => {
    if (!allExistingAgreements.some(existing => 
      existing.email.toLowerCase() === docAgreement.email.toLowerCase() && 
      existing.document_number.toLowerCase() === docAgreement.document_number.toLowerCase()
    )) {
      allExistingAgreements.push(docAgreement);
    }
  });
  
  console.log(`Found ${allExistingAgreements.length} total existing agreements`);
